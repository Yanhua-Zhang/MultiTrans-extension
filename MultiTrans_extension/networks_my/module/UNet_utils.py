    ...
    def __init__(self, in_size, out_size, is_batchnorm, is_ReLU=True, n=2, ks=3, stride=1, padding=1):
        ...
        if is_batchnorm:
            ...
            for i in range(1, n+1):
                ...
                # self.conv1 = conv ~ self.convn = conv
                setattr(self, 'conv%d'%i, conv)
                in_size = out_size
...
class unetUp(nn.Module):
    def __init__(self, in_size, out_size, scale_factor, is_deconv, if_sum_fusion, if_need_channel_change = False, in_size_low_resolution = 125, drop_rate = 0, If_remove_ReLU = False, If_remove_Norm=False):
        ...
        if not self.if_sum_fusion:
            ...
            # padding =1 or feature map will be downsampled 
            self.conv = unetConv2(in_size, out_size, is_batchnorm = True, is_ReLU = not If_remove_ReLU, n=1, ks=3, stride=1, padding=1)
    ...
    def forward(self, inputs1, inputs2):
        ...
        if self.is_deconv:
            outputs2 = self.up(inputs2)  # low resoultion feature upsampling

            # need padding for deconv
...
class UnetGatingSignal3(nn.Module):
    ...
    def forward(self, inputs):
        ...
        outputs = self.conv1(inputs)   # 1x1 conv channel change, then AvgPooling for global feature

        outputs = outputs.view(batch_size, -1)  # 

        outputs = self.fc1(outputs)  # input linear layer
        return outputs
...
class SqEx(nn.Module):
    ...
    def forward(self, x):
        ...
        y = self.nonlin1(self.linear1(y)) # channel reduction
