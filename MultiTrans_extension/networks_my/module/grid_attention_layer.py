...
# need to add final_out_channels=None
class _GridAttentionBlockND(nn.Module):
    def __init__(self, in_channels, gating_channels, inter_channels=None, final_out_channels=None, dimension=3, mode='concatenation',
        ...
        # downsample: stride = kernel size 
        self.theta = conv_nd(in_channels=self.in_channels, out_channels=self.inter_channels,
                             kernel_size=self.sub_sample_kernel_size, stride=self.sub_sample_factor, padding=0, bias=False)

        # 1x1 kernel is linear projection
    ...
    def _concatenation_MultiTrans(self, x, g):
        ...
        # expand_as() the channel to the same as x 
        # pixel-wise multiplication
        y = sigm_psi_f.expand_as(x) * x

        # local feature and global feature fusion 
        fuse = y + g
