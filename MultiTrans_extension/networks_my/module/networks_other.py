...
class SFNet_warp_grid(nn.Module):
    def __init__(self, in_channel, middle_channel):
        ...
        self.offset_map = nn.Conv2d(middle_channel*2, 2, kernel_size=3, padding=1, bias=False)    # for offset map
           
    def forward(self, low_feature, h_feature):
        n, c, h, w = low_feature.size()           # low_feature size
        
        h_feature = self.channel_change1(h_feature)   # channel 
        h_feature_up = F.interpolate(h_feature, (h, w), mode='bilinear', align_corners=True)   
        low_feature = self.channel_change2(low_feature)   # channel 

        fuse_feature = torch.cat([low_feature, h_feature_up], 1)         # 
        flow_field = self.offset_map(fuse_feature)                       # Flow Field：[n,2,h,w]

        norm = torch.tensor([[[[w,h]]]]).type_as(low_feature).to(low_feature.device)   # normalization, grid： -1~1: [1,1,1,2]
        grid_h = torch.linspace(-1,1,h).view(-1,1).repeat(1,w)              # [h,w]
        grid_w = torch.linspace(-1,1,w).repeat(h,1)                         # [h,w]
        grid = torch.cat((grid_w.unsqueeze(2), grid_h.unsqueeze(2)), 2)                           # generate the grid grid_upsample: [h,w,2]  
        grid = grid.repeat(n,1,1,1).type_as(low_feature).to(low_feature.device)    # [n,h,w,2]

        warp_grid = grid + flow_field.permute(0,2,3,1)/norm      # 

        # out_h_feature = F.grid_sample(h_feature_origin, warp_grid)  # feature upsample
        return warp_grid
